  ...
  return (
    <ProtectedRoute>
      <div className="space-y-8">
        <FadeIn className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between border-b border-border/50 pb-6">
          <div>
            <p className="text-xs font-bold text-primary uppercase tracking-widest mb-1">Security & Compliance</p>
            <h1 className="text-4xl font-bold font-heading">Verification Center</h1>
        ...
        {isLoading ? (
          <FadeIn className="rounded-3xl border border-dashed border-slate-200 bg-slate-50/50 p-20 text-center">
            <div className="animate-spin h-8 w-8 border-4 border-primary border-t-transparent rounded-full mx-auto mb-4" />
            <p className="text-slate-500 font-medium">Synchronizing with node...</p>
          </FadeIn>
        ) : (
          <StaggerContainer className="grid gap-8 xl:grid-cols-[1.5fr_1fr]">
            <div className="space-y-8">
                    ...
                    <CardContent className="p-6 space-y-6">
                      {pendingEscrows.length === 0 ? (
                        ...
                        pendingEscrows.map((escrow) => (
                          <div key={escrow.id} className="rounded-2xl border border-border/50 bg-white/40 p-5 group hover:bg-white transition-all duration-300">
                            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between mb-4">
                              <div>
                                <p className="font-bold text-slate-900 font-heading">{escrow.title}</p>
                                <p className="text-xs text-slate-500 font-medium uppercase tracking-tighter">CLIENT ID: {escrow.clientId}</p>
                              </div>
                              <Badge variant="outline" className="bg-blue-50/50 text-blue-700 border-blue-100 font-bold text-[10px]">
                            ...
                            <div className="grid gap-4 sm:grid-cols-2">
                              {escrow.milestones.map((milestone) => (
                                <div key={milestone.id} className="rounded-xl border border-slate-100 bg-slate-50/50 p-4 hover:border-primary/20 transition-colors">
                                  <div className="flex items-center justify-between gap-2 mb-2">
                                    <p className="font-bold text-sm font-heading">{milestone.title}</p>
                                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{milestone.status}</span>
                                  </div>
              ...
              <StaggerItem>
                <Card className="border-border/50 bg-card/50 backdrop-blur-sm">
                  ...
                  <CardContent className="p-6 space-y-6">
                    <div className="flex items-center justify-between gap-4 rounded-2xl border border-border/50 bg-white/40 p-5">
                      <div>
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Network Health</p>
                        <p className="text-2xl font-bold font-heading text-slate-900">{disputedEscrows.length} Active Disputes</p>
                      </div>
                    ...
                    <div className="space-y-4 pt-2">
                      <div className="flex items-start gap-3 p-3 rounded-xl hover:bg-slate-50 transition-colors">
                        ...
                        <div>
                          <p className="text-sm font-bold text-slate-900 font-heading">Automated AI Oracle</p>
                          <p className="text-xs text-slate-500 mt-0.5">Verification scores are computed by decentralized LLM nodes upon proof submission.</p>
                        </div>
                      </div>
                      <div className="flex items-start gap-3 p-3 rounded-xl hover:bg-slate-50 transition-colors">
                        ...
                        <div>
                          <p className="text-sm font-bold text-slate-900 font-heading">Human-in-the-Loop</p>
                          <p className="text-xs text-slate-500 mt-0.5">Complex disputes are escalated to human arbitrators to maintain protocol integrity.</p>
                        </div>
                      </div>
                    </div>
                  </CardContent>
            ...
            <div className="space-y-8">
              <StaggerItem>
                <Card className="border-border/50 bg-card/50 backdrop-blur-sm overflow-hidden">
                  <CardHeader className="bg-rose-50/30 border-b border-rose-100">
                    <CardTitle className="flex items-center gap-2 font-heading text-rose-900">
                  ...
                  <CardContent className="p-6 space-y-4">
                    {disputedEscrows.length === 0 ? (
                      ...
                      disputedEscrows.map((escrow) => (
                        <div key={escrow.id} className="rounded-2xl border border-rose-100 bg-white/40 p-5 group hover:bg-white transition-all">
                          <div className="flex items-center justify-between gap-3 mb-4">
                            <div>
                              <p className="font-bold text-slate-900 font-heading">{escrow.title}</p>
                              <p className="text-[10px] font-bold text-rose-500 uppercase">INITIATED BY {escrow.disputeInitiator}</p>
                            </div>
                            <Badge variant="destructive" className="font-bold text-[10px]">DISPUTED</Badge>
                          </div>
              ...
              <StaggerItem>
                <Card className="border-border/50 bg-slate-900 text-white shadow-xl shadow-slate-200">
                  <CardHeader>
                  ...
                  <CardContent className="p-6 space-y-5">
                    ...
                    <Button variant="outline" className="w-full mt-4 border-slate-700 text-white hover:bg-slate-800 transition-all font-bold">
